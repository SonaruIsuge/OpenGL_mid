...
PlayerShapeA::PlayerShapeA(Shape* parent) : Shape(parent)
{
	_nVtx = 33;

	_Points = new vec4[_nVtx];

	_Points[0] = vec4(-0.1f, 0.4f, 0.0f, 1.0f);
	_Points[1] = vec4(0.0f, 0.5f, 0.0f, 1.0f);
	_Points[2] = vec4(0.1f, 0.4f, 0.0f, 1.0f);

	_Points[3] = vec4(-0.1f, 0.4f, 0.0f, 1.0f);
	_Points[4] = vec4(0.1f, 0.4f, 0.0f, 1.0f);
	_Points[5] = vec4(0.1f, -0.5f, 0.0f, 1.0f);

	_Points[6] = vec4(-0.1f, 0.4f, 0.0f, 1.0f);
	_Points[7] = vec4(0.1f, -0.5f, 0.0f, 1.0f);
	_Points[8] = vec4(-0.1f, -0.5f, 0.0f, 1.0f);

	_Points[9] = vec4(-0.5f, 0.2f, 0.0f, 1.0f);
	_Points[10] = vec4(-0.1f, 0.2f, 0.0f, 1.0f);
	_Points[11] = vec4(-0.1f, 0.0f, 0.0f, 1.0f);

	_Points[12] = vec4(-0.5f, 0.2f, 0.0f, 1.0f);
	_Points[13] = vec4(-0.1f, 0.0f, 0.0f, 1.0f);
	_Points[14] = vec4(-0.5f, 0.1f, 0.0f, 1.0f);

	_Points[15] = vec4(0.1f, 0.2f, 0.0f, 1.0f);
	_Points[16] = vec4(0.5f, 0.2f, 0.0f, 1.0f);
	_Points[17] = vec4(0.5f, 0.1f, 0.0f, 1.0f);

	_Points[18] = vec4(0.1f, 0.2f, 0.0f, 1.0f);
	_Points[19] = vec4(0.5f, 0.1f, 0.0f, 1.0f);
	_Points[20] = vec4(0.1f, 0.0f, 0.0f, 1.0f);

	_Points[21] = vec4(-0.3f, -0.4f, 0.0f, 1.0f);
	_Points[22] = vec4(-0.1f, -0.3f, 0.0f, 1.0f);
	_Points[23] = vec4(-0.1f, -0.5f, 0.0f, 1.0f);

	_Points[24] = vec4(-0.3f, -0.4f, 0.0f, 1.0f);
	_Points[25] = vec4(-0.1f, -0.5f, 0.0f, 1.0f);
	_Points[26] = vec4(-0.3f, -0.5f, 0.0f, 1.0f);

	_Points[27] = vec4(0.1f, -0.3f, 0.0f, 1.0f);
	_Points[28] = vec4(0.3f, -0.4f, 0.0f, 1.0f);
	_Points[29] = vec4(0.3f, -0.5f, 0.0f, 1.0f);

	_Points[30] = vec4(0.1f, -0.3f, 0.0f, 1.0f);
	_Points[31] = vec4(0.3f, -0.5f, 0.0f, 1.0f);
	_Points[32] = vec4(0.1f, -0.5f, 0.0f, 1.0f);
...
PlayerShapeA::~PlayerShapeA(){}


void PlayerShapeA::update(float dt) 
{
