...
void CreateQuadRelationship()
{
	...
	g_pRQuad->setShaderName("vsVtxColor.glsl", "fsVtxColor.glsl");
	...
	for( int i = 0 ; i < 4 ; i++ ) {
		...
		g_pGQuad[i]->setShaderName("vsVtxColor.glsl", "fsVtxColor.glsl");
...
void autoMotion(float delta) {
	
	x = 360 * cosf(g_tMotion * 0.5f * 3.1415926f);
	y = 360 * sinf(g_tMotion * 0.5f * 3.1415926f);
	...
	g_tMotion += delta;
	if (g_tMotion >= 4.0f) g_tMotion -= 4.0f;
}
