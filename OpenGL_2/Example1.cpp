...
void AutomaticRotation(float dt){
	...
	g_TotTime += dt;
	if (g_TotTime >= 4.0f) g_TotTime -= 4.0f;	// 90°/s

	g_fAngle = g_fDir * g_TotTime * 90.0f;
	if (g_iAxis == 0)  mxRot = RotateX(g_fAngle);
	else if (g_iAxis == 1)  mxRot = RotateY(g_fAngle);
	else mxRot = RotateZ(g_fAngle);
	//g_fAngle += g_fDir * g_TotTime * 0.025f;     // 旋轉角度遞增(遞減) 0.025 度
	...
	if (m_bAutoScale) 
	{
		...
		g_fSize = cos(g_ScaTime * PI / 2) > 0 ? cos(g_ScaTime * PI / 2) : -cos(g_ScaTime * PI / 2);
		mxScale = Scale(g_fSize);
	}

	// 計算旋轉矩陣並更新到 Shader 中
	g_pQuad->setTRSMatrix(mxRot * mxScale);
}
//----------------------------------------------------------------------------
